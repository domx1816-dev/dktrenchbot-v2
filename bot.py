...
try:
    import shadow_lane as shadow_lane_mod
    _shadow_lane = shadow_lane_mod.get_shadow_lane()
    _SHADOW_AVAILABLE = True
except Exception as _shadow_import_err:
    _SHADOW_AVAILABLE = False
    logger.debug(f"[shadow] import failed (non-fatal): {_shadow_import_err}")
...
def run_cycle(bot_state: Dict) -> Dict:
    ...
    # ── Shadow lane (non-blocking, every cycle) ───────────────────────────────
    if _SHADOW_AVAILABLE:
        try:
            _shadow_lane.run_cycle_check(candidates, bot_state)
        except Exception as _sle:
            logger.debug(f"[shadow] cycle check error (non-fatal): {_sle}")
    ...
    _now_ts = time.time()
    _cooldowns = {k:v for k,v in _cooldowns.items() if _now_ts - v < COOLDOWN_AFTER_STOP_MIN * 60}
        ...
        for candidate in candidates:
            ...
            try:
                ...
                if chart_state == "orphan":
                    ...
                    if _is_burst and _burst_count >= 8 and chart_state in ("continuation", "expansion", "accumulation"):
                    ...
                    if _SIZING_AVAILABLE:
                        ...
                        if _ML_AVAILABLE:
                            try:
                                ...
                                pass
                        final_size = _calc_position_size(total_score, _ci)
                    else:
            ...
            try:
                ...
                if _is_burst and _burst_count >= 8:
                ...
                # CLOB launch: orderbook buying = real demand, skip price gate
                    ...
                    if len(_prices) >= 3:
                        ...
                        # DATA: stales = 40% of trades. Raised from 1% to 2% — require real movement.
                        if _chg_recent < 2.0:
                            ...
                            if key not in _pending:
                                ...
                                try:
                                    ...
                                    pass
                            logger.info(f"PENDING {symbol}: pre_breakout but price flat ({_chg_recent:+.1f}% recent) — waiting for +2% confirmation")
                            _confirmed = False
                        else:
